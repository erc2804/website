var jumpLocked = false,
    ...
    collIntval = null,
    collidedArr = [
      { id: 1, name: "portfolio", cloudBtm: 80 + "%", url: encodeURI("../../../index.html") },
      { id: 2, name: "socialmedia", cloudBtm: 60 + "%", url: "../../../socialmedia.html" },
      { id: 3, name: "aboutme", cloudBtm: 75 + "%", url: "https://www.linkedin.com/in/ercancicek" }
    ],
    tutorialSeen = true,
    inTutorial = false;
...
function init() {
  ...
  if(!tutorialSeen) { showInstruction(); } else { animateHelpBlock(); }
...
function jumpUp() {
  ...
  collIntval = setInterval(function() {
    collidedArr.forEach(function(collObj, index) {
      ...
      if(collided) {
        ...
        $("#coin_" + collObj.id).removeClass("deepHide").animate({ bottom: collObj.cloudBtm }, { duration: 250, queue: false, done: function() {
  ...
  // --- mel jumps up
  $("#super-mel").animate({ bottom: ((isPortrait() ? _wndw.h : _wndw.w * 0.5) * 0.25) + "px" }, { duration: 300, easing: "swing", queue: false, done: jumpDown });
}

function jumpDown() {
  // --- mel falls back down
  $("#super-mel").animate({ bottom: 10 + "%" }, { duration: 300, easing: "swing", queue: false, done: function() { jumpLocked = false; clearInterval(collIntval); } });
}

function moveMel() {
  // --- mel moves rightwards
  $("#super-mel").animate({ left: 102 + "%" }, { duration: 5000, easing: "linear", queue: false, done: reposMel });
}
...
function setElements() {
  ...
  // --- set block & coin size
  $(".world_block").css({ width: blockSize + "px", height: blockSize + "px", bottom: ((melSize + (isPortrait() ? wndw.h : wndw.w * 0.5) * 0.24) + "px") });
  $(".world_coin").css({ width: blockSize + "px", height: blockSize + "px", bottom: ((melSize + (isPortrait() ? wndw.h : wndw.w * 0.5) * 0.25) + "px") });
